...
fid = open("WritFile.txt",'w')
fid.write('XiaoGongwei PyCharm!')
fid.flush()
fid.close()

import os
mypath = '/home/david/'
pathdisr = os.listdir(mypath)
print(pathdisr)

lsdir = []
for temp in pathdisr:
    allpath = os.path.join(mypath,temp)
    lsdir.append(allpath)
    if True == os.path.isdir(allpath):
        print(allpath,"->is dir")
    else:
        print(allpath,"is\'n dir")
